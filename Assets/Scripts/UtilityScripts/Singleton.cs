...
public class MonoBehaviourSingleton<T> : MonoBehaviour where T : MonoBehaviour
{
    ...
    public  void InitSingleton()
    {
        ...
        if (_instance != null && wasnull)
        {
            if(_instance.transform.parent!=null)
                Debug.Log("[Singleton] Created/found singleton instance (on GO: " + _instance.transform.parent.name+"/"+_instance.name + "): " + typeof(T).Name);
            else
        ...
        {
            if (!((MonoBehaviour)_instance).Equals((MonoBehaviour)this)){
                if(_instance.transform.parent!=null)
                    Debug.Log("[Singleton] Singleton Instance already present: (on GO: " + _instance.transform.parent.name+"/"+_instance.name + "): " + typeof(T).Name+"; Destroying this Mono: "+_instance.transform.parent.name+"/"+this.name);
                else
                    Debug.Log("[Singleton] Singleton Instance already present: (on GO: " + "/"+_instance.name + "): " + typeof(T).Name+"; Destroying this Mono: "+"/"+this.name);
                Destroy(this);
            }else{
                if(_instance.transform.parent!=null)
                    Debug.Log("[Singleton] Singleton Instance already present: (on GO: " + _instance.transform.parent.name+"/"+_instance.name + "): " + typeof(T).Name+";");
                else
    ...
    private static T FindInstance()
    {
        ...
        {
            Debug.LogError("[Singleton] ["+ ((T)objects[0]).name+"] Something went really wrong " +
                " - there should never be more than 1 singleton!" +
                " Reopening the scene might fix it.");
            for(int i = 0; i< objects.Length; i++)
            {
                Debug.LogWarning(((T)objects[i]).name);
            }
            _instance = (T)objects[0];// FindObjectOfType(typeof(T));
            return _instance;
        }
