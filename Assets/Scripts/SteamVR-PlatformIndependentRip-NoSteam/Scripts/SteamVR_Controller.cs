...
public class SteamVR_Controller
{
	...
	public class Device
	{
		...
		public void Update()
		{
			if (Time.frameCount != prevFrameCount)
			{
				...
				if (system != null)
				{
					//TODO: OLD: valid = system.GetControllerStateWithPose(SteamVR_Render.instance.trackingSpace, index, ref state, (uint)System.Runtime.InteropServices.Marshal.SizeOf(typeof(VRControllerState_t)), ref pose);
					UpdateHairTrigger();
				}
			}
		}
		...
		{
			Update();
            var axisId = 0;// (uint)buttonId - (uint)EVRButtonId.k_EButton_Axis0;
			switch (axisId)
			{
	...
	public static int GetDeviceIndex(DeviceRelation relation,
		...
		int relativeTo = -1)//(int)OpenVR.k_unTrackedDeviceIndex_Hmd) // use -1 for absolute tracking space
	{
		var result = -1;
        /*
...
public class ETrackedDeviceClass
{
}