	...
	public class Teleport : MonoBehaviour
    {
        ...
        */
		...
		void Awake()
        {
            ...
            invalidReticleTransform.gameObject.layer = 2;
            destinationReticleTransform.gameObject.layer = 2;
        }
		...
		private void CheckForSpawnPoint(Vector3 forceMoveSpawnPointToHere)
        {
            foreach (TeleportMarkerBase teleportMarker in teleportMarkers)
            {
                ...
                if (teleportPoint && teleportPoint.playerSpawnPoint)
                {
                    teleportPoint.transform.position = forceMoveSpawnPointToHere;
                    teleportingToMarker = teleportMarker;
                    TeleportPlayer();
                    break;
                }
		...
		void Update()
		{
			...
			foreach ( Hand hand in player.hands )
			{
                ...
                //Debug.Log("WasTeleportButtonPressed "+ WasTeleportButtonPressed(hand));
        ...
        /// <summary>
        /// This will overwrite the position of the (first) TeleportPoint Game Object in the scene which is marked as Player Spawn Point.
        /// </summary>
        /// <param name="forceMoveSpawnPointToHere"></param>
        public void TeleportToSpawnPoint(Vector3 forceMoveSpawnPointToHere)
        {
            CheckForSpawnPoint(forceMoveSpawnPointToHere);
        }
        ...
        /// </summary>
        /// <param name="forceMoveSpawnPointToHere"></param>
        public void TryBAMFPlayer(Vector3 target, int handIndex)
        {
            playerSpawnTeleportMarker.transform.position = target;
            forceTeleportToSpawnPoint(handIndex);
        }

        public void TryBAMFPlayer(int handIndex)
        {
            forceTeleportToSpawnPoint(handIndex);
        }
        ...
        public void CancelTeleportHint_OVR()
        {
		...
		private IEnumerator TeleportHintCoroutine()
		{
			...
			while ( true )
			{
				...
				foreach ( Hand hand in player.hands )
				{
                    ...
                    bool isShowingHint = false;// !string.IsNullOrEmpty( ControllerButtonHints.GetActiveHintText( hand, teleportAction) );
					if ( showHint )
					{
						if ( !isShowingHint )
						{
		...
		public bool IsEligibleForTeleport( Hand hand )
		{
			...
			if ( !teleportWhileHovering && hand.hoveringInteractable != null )
			{
				return false;
			}
			...
			if ( hand.noSteamVRFallbackCamera == null )
			{
				...
				if ( !teleportWhileGrabbing && hand.currentAttachedObject != null )
				{
		...
		private bool IsTeleportButtonDown( Hand hand )
		{
			if ( IsEligibleForTeleport( hand ) )
			{
				...
				else
                {
                    ...
                    if (hand.handType == SteamVR_Input_Sources.LeftHand)
                        return OculusInputManager.Instance.LeftTeleportAction;
                    else if (hand.handType == SteamVR_Input_Sources.RightHand)
                        return OculusInputManager.Instance.RightTeleportAction;
                }
			}
