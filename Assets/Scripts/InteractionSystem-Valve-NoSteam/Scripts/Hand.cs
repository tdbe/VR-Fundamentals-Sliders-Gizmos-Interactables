...
namespace Valve.VR.InteractionSystem
{
    ...
    public class Hand : MonoBehaviour
    {
        ...
        /// <summary>
        /// this keeps grabbed objects from receiving the hand focus lost message;
        /// </summary>
        public bool neverTurnOffGrabbedObjects = true;
        ...
        public Collider m_IndexFingerCollider;
        ...
        public bool isActive
        {
            get
            {
                if (trackedObject != null)
                    return false;// trackedObject.isActive;
        ...
        public bool isPoseValid
        {
            get
            {
                return false;// trackedObject.isValid;
            }
        }
        ...
        */
        ...
        public void AttachObject(GameObject objectToAttach, GrabTypes grabbedWithType, AttachmentFlags flags = defaultAttachmentFlags, Transform attachmentOffset = null)
        {
            ...
            if (currentAttachedObject && !neverTurnOffGrabbedObjects)
            {
            ...
            if(objectToAttach.transform.parent != null)
                attachedObject.originalParentIndex = objectToAttach.transform.GetSiblingIndex();
                ...
                if (attachedObject.interactable.attachedToHand != null) //already attached to another hand
                {
                    ...
                    for (int attachedIndex = 0; attachedIndex < attachedObject.interactable.attachedToHand.attachedObjects.Count; attachedIndex++)
                    {
                        ...
                        if (attachedObjectInList.interactable == attachedObject.interactable)
                        {
                            ...
                            attachedObject.originalParentIndex = attachedObjectInList.originalParentIndex;
                        }
                    }
                }
                else
            ...
            {/*
                ...
                { */
                    ...
                    else
                    {
                        ...
                        // Debug.Log("soa Attached: at: "+attachedObject.handAttachmentPointTransform.position);
                        // Debug.Log("soa Attached: obj: "+objectToAttach.transform.position);
                        // Debug.Log("soa Attached: this hand .pos: "+transform.position);
                        // Debug.Log("soa ___");
            ...
            {/*
                ...
                {*/
                    ...
                    else
                    {
                        attachedObject.initialPositionalOffset = attachedObject.handAttachmentPointTransform.InverseTransformPoint(objectToAttach.transform.position);
                        attachedObject.initialRotationalOffset = Quaternion.Inverse(attachedObject.handAttachmentPointTransform.rotation) * objectToAttach.transform.rotation;
                        // Debug.Log("Attached: handAttachmentPointTransform.InverseTransformPoint: "+attachedObject.initialPositionalOffset);
                        // Debug.Log("Attached: obj: "+objectToAttach.transform.position);
                        // Debug.Log("Attached: this hand .pos: "+transform.position);
                        // Debug.Log("___");
            ...
            if (index != -1)
            {
                ...
                if (attachedObjects[index].isParentedToHand)
                {
                    ...
                    if (attachedObjects[index].attachedObject != null)
                    {
                        ...
                        attachedObjects[index].attachedObject.transform.SetSiblingIndex(attachedObjects[index].originalParentIndex);
                ...
                if(index >=0 && index < attachedObjects.Count)
        ...
        public void GetEstimatedPeakVelocities(out Vector3 velocity, out Vector3 angularVelocity)
        {
            ...
            velocity = Player.instance.trackingOriginTransform.TransformVector(Vector3.one);
            angularVelocity = Player.instance.trackingOriginTransform.TransformDirection(Vector3.one);
        }
                    ...
                    */
                    ...
                    if (currentAttachedObjectInfo.Value.interactable.handFollowTransform)
                    {
                        ...
                        //{
                            ...
                            Quaternion rot = mainRenderModel == null? transform.rotation : mainRenderModel.GetHandRotation();
                            Quaternion rotationDiff = rot * Quaternion.Inverse(this.transform.rotation);
        ...
        {/*
            if (attachedObject.interactable != null && attachedObject.interactable.skeletonPoser != null && HasSkeleton())
            {
                ...
                //tp.x *= -1;
                return Vector3.zero;//currentAttachedObjectInfo.Value.handAttachmentPointTransform.TransformPoint(tp);
            }
            else
            {
        ...
        public GrabTypes GetGrabStarting(GrabTypes explicitType = GrabTypes.None)
        {
            if (explicitType != GrabTypes.None)
            {
                ...
                if (explicitType == GrabTypes.Grip && grabGripAction.GetStateDown(handType))
                    ...
                    */
                if (explicitType == GrabTypes.Pinch && OculusInputManager.Instance.GetGrabPinchDown(OculusInputManager.Instance.GetOculusHand(handType)))
                    return GrabTypes.Pinch;
                if (explicitType == GrabTypes.Grip && OculusInputManager.Instance.GetGrabGripDown(OculusInputManager.Instance.GetOculusHand(handType)))
                    return GrabTypes.Grip;
            }
            else
            {
                ...
                if (grabGripAction.GetStateDown(handType))
                    return GrabTypes.Grip;
                    */
                if (OculusInputManager.Instance.GetGrabPinchDown(OculusInputManager.Instance.GetOculusHand(handType)))
                    return GrabTypes.Pinch;
                if (OculusInputManager.Instance.GetGrabGripDown(OculusInputManager.Instance.GetOculusHand(handType)))
                    return GrabTypes.Grip;
        ...
        public GrabTypes GetBestGrabbingType(GrabTypes preferred, bool forcePreference = false)
        {
            ...
            if (preferred == GrabTypes.Pinch)
            {
                if (OculusInputManager.Instance.GetGrabPinch(OculusInputManager.Instance.GetOculusHand(handType)))
                    return GrabTypes.Pinch;
                else if (forcePreference)
                    return GrabTypes.None;
            }
            if (preferred == GrabTypes.Grip)
            {
                if (OculusInputManager.Instance.GetGrabGrip(OculusInputManager.Instance.GetOculusHand(handType)))
                    return GrabTypes.Grip;
                else if (forcePreference)
                    return GrabTypes.None;
            }
        ...
        {
            return 0;// trackedObject.GetDeviceIndex();
        }
    }
