...
public class EnalargeDebounced : MonoBehaviour
{
    ...
    Vector3 m_originalSize = Vector3.one;
    ...
    IEnumerator ShrinkCor(System.Action act){
    ...
    public void ShrinkBackAndDisableGO(){
        if(m_corE!=null){
            StopCoroutine(m_corE);
        }
        if(Vector3.Distance(transform.localScale,m_originalSize)>0.01f)
            m_corS = StartCoroutine(ShrinkCor(()=>{gameObject.SetActive(false);}));
        else
            transform.localScale = m_originalSize;
    }
